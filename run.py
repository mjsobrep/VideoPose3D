...
dataset = CustomDataset(args.dataset + '.npz')
...
keypoints = np.load(args.dataset + '.npz', allow_pickle=True)
...
if not args.evaluate:
    ...
    if semi_supervised:
        ...
        semi_generator_eval = UnchunkedGenerator(cameras_semi, None, poses_semi_2d,
                                                 pad=pad, causal_shift=causal_shift, augment=False)
        print(
            'INFO: Semi-supervision on {} frames'.format(semi_generator_eval.num_frames()))
    ...
    while epoch < args.epochs:
        ...
        if semi_supervised:
            ...
            for (_, batch_3d, batch_2d), (cam_semi, _, batch_2d_semi) in \
                ...
                inputs_2d_semi = torch.from_numpy(
                    batch_2d_semi.astype('float32'))
                if torch.cuda.is_available():
                    inputs_2d = inputs_2d.cuda()
                    inputs_2d_semi = inputs_2d_semi.cuda()
                inputs_2d_cat = torch.cat(
                    (inputs_2d, inputs_2d_semi), dim=0) if not skip else inputs_2d
                ...
                loss_3d_pos = mpjpe(
                    predicted_3d_pos_cat[:split_idx], inputs_3d)
                ...
                epoch_loss_traj_train += inputs_3d.shape[0] * \
                    inputs_3d.shape[1] * loss_traj.item()
                assert inputs_traj.shape[0] * \
                    inputs_traj.shape[1] == inputs_3d.shape[0] * \
                    inputs_3d.shape[1]
                ...
                if not skip:
                    ...
                    loss_reconstruction = mpjpe(
                        reconstruction_semi, target_semi)  # On 2D poses
                    epoch_loss_2d_train_unlabeled += predicted_semi.shape[0] * \
                        predicted_semi.shape[1] * loss_reconstruction.item()
                    if not args.no_proj:
                    ...
                    if args.bone_length_term:
                        dists = predicted_3d_pos_cat[:, :, 1:] - \
                            predicted_3d_pos_cat[:, :, dataset.skeleton().parents()[
                                1:]]
                        bone_lengths = torch.mean(
                            torch.norm(dists, dim=3), dim=1)
            ...
            losses_2d_train_unlabeled.append(
                epoch_loss_2d_train_unlabeled / N_semi)
        else:
            ...
            if not args.no_eval:
                ...
                for cam, batch, batch_2d in test_generator.next_epoch():
                    ...
                    if semi_supervised:
                        ...
                        epoch_loss_traj_valid += inputs_traj.shape[0] * \
                            inputs_traj.shape[1] * loss_traj.item()
                        assert inputs_traj.shape[0] * \
                            inputs_traj.shape[1] == inputs_3d.shape[0] * \
                            inputs_3d.shape[1]
                        ...
                        loss_reconstruction = mpjpe(
                            reconstruction, target)  # On 2D poses
                        epoch_loss_2d_valid += reconstruction.shape[0] * \
                            reconstruction.shape[1] * \
                            loss_reconstruction.item()
                        assert reconstruction.shape[0] * \
                            reconstruction.shape[1] == inputs_3d.shape[0] * \
                            inputs_3d.shape[1]
                ...
                for cam, batch, batch_2d in train_generator_eval.next_epoch():
                    ...
                    if semi_supervised:
                        ...
                        predicted_traj = model_traj(inputs_2d)
                        loss_traj = mpjpe(predicted_traj, inputs_traj)
                        epoch_loss_traj_train_eval += inputs_traj.shape[0] * \
                            inputs_traj.shape[1] * loss_traj.item()
                        assert inputs_traj.shape[0] * \
                            inputs_traj.shape[1] == inputs_3d.shape[0] * \
                            inputs_3d.shape[1]
                        ...
                        epoch_loss_2d_train_labeled_eval += reconstruction.shape[0] * \
                            reconstruction.shape[1] * \
                            loss_reconstruction.item()
                        assert reconstruction.shape[0] * \
                            reconstruction.shape[1] == inputs_3d.shape[0] * \
                ...
                if semi_supervised:
                    losses_traj_train_eval.append(
                        epoch_loss_traj_train_eval / N)
                ...
                if semi_supervised:
                    for cam, _, batch_2d in semi_generator_eval.next_epoch():
                        ...
                        N_semi += reconstruction_semi.shape[0] * \
        ...
        momentum = initial_momentum * \
            np.exp(-epoch/args.epochs * np.log(initial_momentum/final_momentum))
        ...
        if args.export_training_curves and epoch > 3:
            ...
            if semi_supervised:
                ...
                plt.plot(
                    epoch_x, losses_2d_train_unlabeled[3:], '--', color='C1')
                plt.plot(
                    epoch_x, losses_2d_train_unlabeled_eval[3:], color='C1')
